...
\node[draw, rounded corners, fill=green!10, text width=7cm] at (4.5,6) {
    ...
    $\checkmark$ Natural heterogeneity representation\\
    $\checkmark$ Spatial dynamics and local interactions\\
    $\checkmark$ Stochasticity at individual level\\
    $\checkmark$ Easy addition of new cell types\\
    $\checkmark$ Direct biological interpretation\\
    $\checkmark$ Modular/extensible architecture
};

% Disadvantages
\node[draw, rounded corners, fill=red!10, text width=7cm] at (4.5,4.5) {
    \textbf{ABM Disadvantages}\\
    $\times$ Computationally expensive\\
    $\times$ Many parameters to calibrate\\
    $\times$ Stochastic results require ensembles\\
    $\times$ Difficult mathematical analysis\\
    $\times$ Scale limitations (memory/time)
};
...
\node[draw, rounded corners, fill=green!10, text width=7cm] at (13.5,6) {
    ...
    $\checkmark$ Mathematically tractable\\
    $\checkmark$ Fast computation\\
    $\checkmark$ Analytical solutions possible\\
    $\checkmark$ Parameter sensitivity analysis\\
    $\checkmark$ Stability/bifurcation analysis\\
    $\checkmark$ Well-established theory
};

% Disadvantages
\node[draw, rounded corners, fill=red!10, text width=7cm] at (13.5,4.5) {
    \textbf{ODE/PDE Disadvantages}\\
    $\times$ No individual heterogeneity\\
    $\times$ Limited spatial resolution\\
    $\times$ Difficult to add discrete events\\
    $\times$ Mean-field assumptions\\
    $\times$ Less intuitive for cell biology
};
