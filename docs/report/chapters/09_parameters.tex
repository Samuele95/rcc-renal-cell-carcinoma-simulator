...
The model employs a 114-parameter hierarchy: 4 model parameters (volume, resolution, seed), 18 patient parameters (clinical variables), and 92 weight parameters (biological process rates) optimized via Bayesian methods against clinical trial data.

% ======================================================================
\section{Parameter Categories}
\label{sec:param-categories}

Parameters are organized into three dataclass objects: ModelParameters, PatientParameters, and WeightParameters.

ModelParameters: 4 simulation configuration parameters (volume, block size, random seed, max steps). PatientParameters: 18 patient-specific parameters (sex, BMI, treatment type, start time, plus 14 immune cell concentrations). WeightParameters: 92 biological process rate parameters optimized via Bayesian methods.

% ======================================================================
\section{Weight Parameters}
\label{sec:weight-params}

The 84 weight parameters control probability thresholds and scaling factors for all agent behaviors, organized by cell type. These parameters are optimized via Bayesian methods to match clinical trial outcomes.
...
With 92 parameters tuned against clinical reality, we can finally ask the question that motivated this entire project: does sex matter in RCC treatment? The next chapter reveals surprising answers that challenge conventional treatment wisdom.
