...
\begin{figure}[htbp]
\centering
% TikZ figure temporarily disabled - simulation pipeline would show step sequence
\textit{[Simulation pipeline diagram showing the 11-step sequence: collect\_data $\rightarrow$ check\_termination $\rightarrow$ apply\_effects $\rightarrow$ treatment $\rightarrow$ sex\_drift $\rightarrow$ glucose\_step $\rightarrow$ manage\_blood $\rightarrow$ agent\_steps $\rightarrow$ move $\rightarrow$ spawn\_hormones]}
