...
\setbeamercolor{progress bar}{fg=tealblue,bg=white}
\setbeamercolor{frametitle}{bg=tealblue}
...
\setbeamercolor{progress bar in head/foot}{fg=tealblue,bg=white}
\setbeamercolor{standout}{bg=tealblue}
...
\setbeamercolor{block body alerted}{bg=red!20!white,fg=black}
\setbeamercolor{block title example}{bg=green!70!black,fg=white}
\setbeamercolor{block body example}{bg=green!20!white,fg=black}
...
% Progress bar
\makeatletter
\setlength{\metropolis@progressinheadfoot}{2pt}
\makeatother
...
% ---- Dramatic section title slides ----
\AtBeginSection[]{%
  \begin{frame}[plain]
    \begin{tikzpicture}[remember picture,overlay]
      \fill[tealblue] (current page.south west) rectangle (current page.north east);
      \node[white, font=\Huge\bfseries] at (current page.center) {\insertsectionhead};
    \end{tikzpicture}
  \end{frame}
}
...
% ---- Slide 2b: THE NUMBERS (High Impact) ----
\begin{frame}[standout]
  \begin{center}
    {\Huge\textbf{THE NUMBERS}}
    
    \vspace{1em}
    
    {\Large\color{white}\textbf{400,000}} \\
    \textit{new RCC cases per year}
    
    \vspace{0.8em}
    
    {\Large\color{white}\textbf{<15\%}} \\
    \textit{metastatic survival rate}
    
    \vspace{0.8em}
    
    {\Large\color{white}\textbf{18}} \\
    \textit{agent types modeled}
    
    \vspace{0.8em}
    
    {\Large\color{white}\textbf{160}} \\
    \textit{simulations run}
    
    \vspace{0.8em}
    
    {\Large\color{white}\textbf{65\%}} \\
    \textit{best survival rate achieved}
  \end{center}
\end{frame}
...
\begin{frame}{Renal Cell Carcinoma — The Clinical Challenge}
  \begin{columns}[T]
    ...
    \begin{itemize}
      \item {\Large\color{tealblue}\textbf{3rd}} most common urological cancer
      \item {\Large\color{tealblue}\textbf{400,000}} new cases per year
      \item Metastatic survival: {\Large\color{red!80!black}\textbf{<15\%}}
      \item Clear-cell RCC: {\Large\color{tealblue}\textbf{75\%}} of cases
      \item Sex ratio: {\Large\color{tealblue}\textbf{2:1}} (M:F)
      \item Females show \textbf{better immunotherapy response}
    \end{itemize}

    \column{0.38\textwidth}
    \begin{alertblock}{Crisis}
      \centering
      {\Large\color{white}\textbf{1 in 6}}\\
      \textit{patients survive 5 years}\\
      \textit{after metastasis}
    \end{alertblock}
    
    \vspace{0.5em}
    
    \begin{exampleblock}{Hope}
      \centering
      {\Large\color{white}\textbf{ICI + TKI}}\\
      \textit{combination therapy}\\
      \textit{shows promise}
    \end{exampleblock}
  \end{columns}
\end{frame}
...
\begin{frame}{Why Agent-Based Modeling?}
  \begin{columns}[T]
    ...
    \begin{exampleblock}{ABM Advantages}
      \begin{itemize}
        \item \textbf{Heterogeneous} cell populations
        \item \textbf{Spatial dynamics} \& local interactions
        \item \textbf{Emergent} behavior from simple rules
        \item \textbf{Fast, ethical, cheap} experiments
        \item Natural stochasticity
      \end{itemize}
    \end{exampleblock}

    \column{0.48\textwidth}
    \begin{alertblock}{ODE/PDE Limitations}
      \begin{itemize}
        \item Well-mixed assumptions
        \item No cell-level heterogeneity
        \item No spatial resolution
        \item Discrete events difficult
      \end{itemize}
    \end{alertblock}
  \end{columns}
  ...
  \vspace{1em}
  \centering
  \textit{"The whole is greater than the sum of its parts"} — ABMs capture tumor ecosystem complexity
\end{frame}

% ---- Slide 5: Project Objectives ----
\begin{frame}{Project Objectives}
  \begin{enumerate}
    \item \textbf{Mesa → Repast4Py Migration}\\
          \textit{Scalable MPI-based framework}
          
    \item \textbf{Glucose Microenvironment}\\
          \textit{3D Warburg effect modeling}
          
    \item \textbf{Clinical Validation}\\
          \textit{ARON-1 survival data fitting}
          
    \item \textbf{Bayesian Optimization}\\
          \textit{{\color{tealblue}\textbf{114}} parameters, {\color{tealblue}\textbf{250}} trials}
          
    \item \textbf{Sex-Specific Analysis}\\
          \textit{Male vs Female immune dynamics}
  \end{enumerate}
  
  \vspace{0.8em}
  \centering
  \textbf{Goal:} Professional-grade cancer simulation platform
\end{frame}
...
\begin{frame}{ABM Formalism \& Repast4Py}
  \begin{columns}[T]
    ...
    \begin{block}{ABM Components}
      \begin{description}
        ...
        \item[Environment] 3D grid + glucose field
        \item[Rules] step functions each tick
        \item[Emergence] macro from micro interactions
      \end{description}
    \end{block}

    \column{0.48\textwidth}
    \begin{block}{Repast4Py Architecture}
      \small
      \begin{itemize}
        \item \texttt{Agent} — base class
        \item \texttt{SharedContext} — distributed agents
        \item \texttt{SharedGrid} — MPI 3D grid
        \item \texttt{BorderType.Sticky} — boundaries
        \item \texttt{OccupancyType.Multiple} — co-location
        \item \texttt{mpi4py.MPI} — communication
      \end{itemize}
    \end{block}
  \end{columns}
  ...
  \vspace{1em}
  \centering
  {\large From microscopic cell behavior → macroscopic tumor dynamics}
\end{frame}

% ---- Slide 7: Immune System Overview ----
\begin{frame}{Immune System — {\Large\color{tealblue}\textbf{18}} Agent Types}
  \begin{columns}[T]
    \column{0.48\textwidth}
    \begin{block}{Innate Immunity}
      \begin{itemize}
        \item \textbf{M1} — phagocytosis, inflammation
        \item \textbf{M2} — repair, pro-tumor
        \item \textbf{NK} — direct killing
        \item \textbf{DC} — antigen presentation
        \item \textbf{pDC} — interferon production
        \item \textbf{Mast} — angiogenesis
    ...
    \begin{block}{Adaptive Immunity}
      \begin{itemize}
        \item \textbf{CD8+ CTL} — tumor killing
        \item \textbf{CD8+ Naïve} — CTL precursors
        \item \textbf{CD4+ Naïve} — helper precursors
        \item \textbf{Th1} — anti-tumor response
        \item \textbf{Th2} — humoral immunity
        \item \textbf{Treg} — immune suppression
      \end{itemize}
    \end{block}
  \end{columns}
  ...
  \begin{alertblock}{}
    \centering
    \textbf{Plus:} Tumor cells • Adipocytes • Blood vessels • Sex hormones • Cytokines
  \end{alertblock}
\end{frame}

% ---- Slide 8: Warburg Effect ----
\begin{frame}{The Warburg Effect — Metabolic Warfare}
  \begin{columns}[T]
    \column{0.55\textwidth}
    \begin{itemize}
      \item Tumor cells use \textbf{aerobic glycolysis}
      \item Consumption: {\Large\color{red!80!black}\textbf{10–200×}} higher
      \item Creates \textbf{metabolic competition}
      \item Starves immune cells of glucose:
        \begin{itemize}
          \item X T cell activation
          \item X NK cytotoxicity  
          \item X DC presentation
        \end{itemize}
      \item Glucose = \textbf{limiting factor}
    \end{itemize}

    \column{0.42\textwidth}
    \begin{alertblock}{Metabolic War}
      \centering
      {\large \textbf{GLUCOSE}}\\
      \vspace{0.5em}
      \textbf{Tumor}: high consumption\\
      \textbf{Immune}: starved
    \end{alertblock}
    
    \vspace{0.5em}
    
    \begin{tikzpicture}[scale=0.8, every node/.style={scale=0.8}]
      \node[draw, rounded corners, fill=red!15, minimum width=2.8cm, minimum height=1cm] (tumor) at (0,2.5) {Tumor Cell};
      ...
      \draw[-{Stealth[length=3mm]}, thick, red!70!black] (glucose.west) -- node[above,sloped,font=\scriptsize]{HIGH} (tumor.east);
      \draw[-{Stealth[length=3mm]}, thick, blue!70!black] (glucose.west) -- node[below,sloped,font=\scriptsize]{low} (immune.east);
...
% ---- WHY IT MATTERS (High Impact) ----
\begin{frame}[standout]
  \begin{center}
    {\Huge\textbf{WHY IT MATTERS}}
    
    \vspace{1.5em}
    
    {\Large\color{white} Behind every simulation...}
    
    \vspace{0.8em}
    
    {\Large\color{white}\textbf{400,000}} \textit{real patients per year}
    
    \vspace{0.8em}
    
    {\Large\color{white}\textbf{Families}} \textit{waiting for answers}
    
    \vspace{0.8em}
    
    {\Large\color{white}\textbf{Digital twins}} \textit{can guide therapy}
    
    \vspace{1em}
    
    \textit{Computational oncology saves lives}
  \end{center}
\end{frame}
...
\begin{frame}[standout]
  \begin{center}
    ...
    {\Large\color{white} Streamlit Web Interface}
    
    \vspace{0.8em}
    
    {\Large\color{white} Real-time Visualization}
    
    \vspace{0.8em}
    
    {\Large\color{white} Parameter Tuning}
    
    \vspace{0.8em}
    
    {\Large\color{white} Interactive Results}
    
    \vspace{1em}
    
    \textit{Ready to explore the tumor ecosystem?}
  \end{center}
\end{frame}
